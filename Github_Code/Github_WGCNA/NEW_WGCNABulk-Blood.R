...
allowWGCNAThreads()
#------------------------------read blood, paclitaxel AND paclitaxel plus atezolizumab-------------------------------------
folder_names <- c("Post_P001_b", "Post_P005_b", "Post_P019_b", "Prog_P008_b", 
                  "Pre_P023_b", "Pre_P024_b", "Pre_P025_b", "Pre_P001_b", 
                  "Pre_P002_b", "Pre_P004_b", "Pre_P005_b", "Pre_P007_b", 
                  "Pre_P019_b", "Prog_P002_b", "Prog_P004_b", "Prog_P007_b", 
                  "Post_P011_b", "Post_P023_b", "Post_P024_b", "Post_P025_b", 
                  "Pre_P008_b", "Pre_P011_b")
...
prepare_sft_plot <- function(sft) {
  ...
  selected_power <- df$Power[which.max(df$SFT_R2)]
...
summary(pre_result$df$SFT_R2)
pre_result$df[which.max(pre_result$df$SFT_R2), ]
...
ggsave("F:\\WGCNA\\Important_modules_Blood\\Important_modules-Results\\pre_soft_thresholding_plot.png", width=8, height=6, dpi=300)
# رسم نمودار برای post
...
ggsave("F:\\WGCNA\\Important_modules_Blood\\Important_modules-Results\\post_soft_thresholding_plot.png", width=8, height=6, dpi=300)
...
ggsave("F:\\WGCNA\\Important_modules_Blood\\Important_modules-Results\\dendrogram_pre.png", width=8, height=6, dpi=300)
dev.off()
...
plotDendroAndColors(geneTree_post, moduleColors_post,"Module colors (Post)",dendroLabels = FALSE,
                    hang = 0.03,addGuide = TRUE, guideHang = 0.05)
ggsave("F:\\WGCNA\\Important_modules_Blood\\Important_modules-Results\\dendrogram_post.png", width=8, height=6, dpi=300)
dev.off()
# ============================eigen gene
...
important_modules
important_module_colors <- names(MEs_pr)[important_modules]
important_module_colors
module_importance
summary(module_importance)
#============================= Save the Workplace
save.image("F:\\WGCNA\\module_importance_workspace.RData")
save(MEs_pr, dynamicMods_pre, file = "F:\\WGCNA\\MEs_pr_results.RData")
save(MEs_post, dynamicMods_pre, file = "F:\\WGCNA\\MEs_post_results.RData")

load("F:\\WGCNA\\module_importance_workspace.RData")
load("F:\\WGCNA\\MEs_pr_results.RData")
load("F:\\WGCNA\\MEs_post_results.RData")
