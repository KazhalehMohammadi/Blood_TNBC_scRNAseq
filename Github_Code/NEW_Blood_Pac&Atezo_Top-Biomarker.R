...
# ===============================================
g2 <- process_group("PBMC_PacAtezo",
                    "F:\\Blood-Patient-Paclitaxel-plus atezolizumab-Analysis\\Figs\\TOP10-Clusters-final-Blood-Pacli&Atezo-plus Azi.csv",
                    "F:\\WGCNA\\Blood-Patient-Paclitaxel&Atizo-Analysis\\Results\\Max_Module_Membership_Per_Gene.csv",
                    "F:\\Blood-Patient-Paclitaxel-plus atezolizumab-Analysis\\Figs\\Enrich\\GO_Enrichment.csv",
                    "F:\\Blood-Patient-Paclitaxel-plus atezolizumab-Analysis\\Figs\\Enrich\\KEGG_Enrichment.csv"
)
all_biomarkers <- bind_rows(g2)
outdir <- "F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo"

write_csv(all_biomarkers, file.path(outdir, "All_Biomarkers_AllGroups.csv"))
# ===============================================
outdir <- "F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo"
write_csv(all_biomarkers, "F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\All_Biomarkers_AllGroups.csv")
...
dev.off()
# ======================================
df <- readr::read_csv("F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\Top80_Biomarkers_WeightedScore.csv")
df <- as.data.frame(df)
...
ggsave("F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\Top20_Biomarkers_Barplot_Weighted_FIXED.png",
       plot = p, width = 9, height = 6, dpi = 600)
# ==========
top_file <- "F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\Top80_Biomarkers_WeightedScore.csv"
df <- readr::read_csv(top_file)
genes <- df %>%
...
head(kegg@result)
write.csv(kegg@result, "F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\KEGG_Results.csv", row.names = FALSE)
kegg_pathways_genes <- kegg@result %>%
  dplyr::filter(p.adjust < 0.1) %>%
  dplyr::select(Description, geneID) %>%
  dplyr::mutate(Genes = gsub("/", ", ", geneID)) %>%
  dplyr::select(-geneID)
write.csv(kegg_pathways_genes, "F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\KEGG_Pathway_GeneList.csv", row.names = FALSE)
if (nrow(kegg@result) > 0) {
  ggsave("F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\KEGG_barplot_top40_filtered.pdf",
...
ggsave("F:\\TOP-BIOMARKERS\\Blood_Pac&Atezo\\Key_GO_Biological_Processes_Enriched_in_Top_40_Biomarkers.pdf",
       plot = p_go,  
